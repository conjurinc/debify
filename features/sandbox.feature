...
Feature: Running a sandbox
  ...
  Scenario: sandbox for 'example' project be started linked to another container
    ...
    Then I successfully start a sandbox for "example" with arguments "-t 5.0-stable --no-pull --link other_host -c 'ping -c1 other_host'"
  ...
  Scenario: sandbox for 'example' project be started on a network other than the default
    ...
    Then I successfully start a sandbox for "example" with arguments "-t 5.0-stable --no-pull --net test-net -c 'ping -c1 other_host'"
  ...
  Scenario: sandbox for 'example' project be started on a network other than the default with a host aliased
    ...
    Then I successfully start a sandbox for "example" with arguments "-t 5.0-stable --no-pull --net test-net --link another_host:other_host -c 'ping -c1 other_host'"
